  ...
  <div>
        ...
        <div class="item-group" v-for="item in groupList" :key="item.GroupId">
          <div class="item-group-title">
            ...
            <span class="item-group-title-right">
              <el-button @click="showEditItemDialog(item)" type="text">编 辑</el-button>
              <el-button @click="deleteItem(item)" type="text" class="red">删 除</el-button>
            </span>
          </div>
          <div class="item-group-content">
            <span v-for="x in item.ItemList" :key="x.SheetItemRelId" class="item">
    ...
    <div class="sheetItemFrame">
      <div class="sheetItemTitle">
        ...
        <div class="right-button">
          <el-button @click="showCopyDialog(1)" type="primary" size="medium">批量拷贝</el-button>
          <el-button @click="showAddBasePriceDialog()" type="primary" size="medium">添加价格</el-button>
        </div>
      </div>
      <el-table :data="basePriceTable" border ref="multipleTable">
        ...
        <el-table-column align="center" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" @click="showEditBasePriceDialog(scope.row)">编辑</el-button>
            <el-button type="text" @click="deleteBasePrice(scope.row)" class="red">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="sheetItemFrame">
      ...
      <el-table :data="specialPriceTable" border ref="multipleTable">
        ...
        <el-table-column align="center" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" @click="showEditSpecialPriceDialog(scope.row)">编辑</el-button>
            <el-button type="text" @click="deleteSpecialPrice(scope.row)" class="red">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="sheetItemFrame">
      <div class="sheetItemTitle">
        验券后退款规则
        <div class="right-button">
          <el-button @click="showCopyDialog(2)" type="primary" size="medium">批量拷贝</el-button>
          <el-button
            class="right-button"
      ...
      <el-table :data="refundRulesTable" border ref="multipleTable">
        ...
        <el-table-column align="center" label="操作" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="showEditRefundRulesDialog(scope.row)">编辑</el-button>
            <el-button type="text" @click="deleteRefundRules(scope.row)" class="red">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
      ...
      <div class="sheetItemTitle">
        ...
        <div class="right-button">
          <el-button
            ...
            type="primary"
            size="medium"
            v-if="prizeRulesTable.length>0"
          >初始化特殊价格</el-button>
          <el-button @click="showCopyDialog(3)" type="primary" size="medium">批量拷贝</el-button>
          <el-button
            class="right-button"
      ...
      <el-table :data="prizeRulesTable" border ref="multipleTable">
        ...
        <el-table-column align="center" label="操作" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="showEditPrizeRulesDialog(scope.row)">编辑</el-button>
            <el-button type="text" @click="deletePrizeRules(scope.row)" class="red">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <!-- 添加项目弹窗 -->
    <el-dialog title="添加课程" :visible.sync="addItemVisible" width="930px">
        ...
        <el-form-item label="课程列表:">
          <div class="item-border">
            <el-checkbox-group v-model="addItemForm.itemCheckList">
              <div class="item-group-content">
                <div v-for="item in allItemList" :key="item.Id" class="item">
                  <img :src="item.ItemCoverShowImg" />
                  <div>
                    <el-checkbox :label="item.Id" :disabled="item.Disabled==true">{{item.ItemName}}</el-checkbox>
      ...
      <span slot="footer" class="dialog-footer">
        <el-checkbox v-model="addItemForm.tolerate" style="float:left;margin-top:10px;margin-left:20px;">商家端默认团单分类</el-checkbox>
    ...
    >
      <el-form ref="editItemForm" :model="editItemForm" label-width="100px" :rules="ItemFormRules">
        ...
        <br />
        <el-checkbox v-model="editItemForm.tolerate">商家端默认团单分类</el-checkbox>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    showAddItemDialog() {
      ...
      this.groupId = -1;
      this._getActivityGroupStoreList();
    ...
    _getSheetItemGroupList() {
      ...
      return GetSheetItemGroupList(par).then(res => {
        this.groupList = res;
      });
    },
    //获取团单基础价格列表
    _getSheetNormalPriceList() {
      ...
      return GetSheetNormalPriceList(par).then(res => {
    ...
    _getSheetSpecialPriceList() {
      ...
      return GetSheetSpecialPriceList(par).then(res => {
    ...
    _getSheetRefundRulesList() {
      ...
      return GetSheetRefundRulesList(par).then(res => {
    ...
    _getSheetPrizeRulesList() {
      ...
      return GetSheetPrizeRulesList(par).then(res => {
        this.prizeRulesTable = res;
      });
    },
    //获取活动组参与门店列表
    _getActivityGroupStoreList() {
      var par = {
        ActivityGroupId: this.activityGroupId,
        StoreName: "",
        PageIndex: 1,
      ...
      return activitygroupStoreList(par).then(res => {
    ...
    _modifySheetItemGroup() {
      ...
      return ModifySheetItemGroup(par);
    },
    //删除团单项目分组
    _deleteSheetItemGroup(item) {
      ...
      return DeleteSheetItemGroup(par);
    },
    //添加团单基础价格
    _addSheetNormalPrice() {
      ...
      return AddSheetNormalPrice(par);
    },
    //编辑团单基础价格
    _modifySheetNormalPrice() {
      ...
      return ModifySheetNormalPrice(par);
    },
    //删除团单基础价格
    _deleteSheetNormalPrice(item) {
      ...
      return DeleteSheetNormalPrice(par);
    },
    //修改团单基础价格状态
    _modifySheetNormalPriceStatus(id, status) {
      ...
      return ModifySheetNormalPriceStatus(par);
    },
    //添加团单特殊价格
    _addSheetSpecialPrice() {
      ...
      return AddSheetSpecialPrice(par);
    },
    //编辑团单特殊价格
    _modifySheetSpecialPrice() {
      ...
      return ModifySheetSpecialPrice(par);
    },
    //删除团单特殊价格
    _deleteSheetSpecialPrice(item) {
      ...
      return DeleteSheetSpecialPrice(par);
    },
    //修改团单特殊价格状态
    _modifySheetSpecialPriceStatus(id, status) {
      ...
      return ModifySheetSpecialPriceStatus(par);
    },
    //添加团单退款规则
    _addSheetRefundRules() {
      ...
      return AddSheetRefundRules(par);
    },
    //编辑团单退款规则
    _modifySheetRefundRules() {
      ...
      return ModifySheetRefundRules(par);
    },
    //删除团单退款规则
    _deleteSheetRefundRules(item) {
      ...
      return DeleteSheetRefundRules(par);
    },
    //修改团单退款规则状态
    _modifySheetRefundRulesStatus(id, status) {
      ...
      return ModifySheetRefundRulesStatus(par);
    },
    //添加团单奖励规则
    _addSheetPrizeRules() {
      var par = {
    ...
    _modifySheetPrizeRules() {
      ...
      return ModifySheetPrizeRules(par);
    },
    //删除团单奖励规则
    _deleteSheetPrizeRules(item) {
      ...
      return DeleteSheetPrizeRules(par);
    },
    //修改团单奖励规则状态
    _modifySheetPrizeRulesStatus(id, status) {
      ...
      return ModifySheetPrizeRulesStatus(par);
    },
    //获取团单列表
    _getSheetList() {
      var par = {
    ...
    _prizeSpecialPriceInit() {
      ...
      return PrizeSpecialPriceInit(par);
    }
  }
};
</script>
