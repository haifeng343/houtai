  ...
  <div>
    ...
    >
      ...
      <div class="form3" v-show="showData3">
        ...
        <a href="javascript:;" class="addIcon" @click="addDate" v-show="showAdd">+</a>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      addrules: {
        ...
        ItemId: [{ required: true, message: "请选择课程", trigger: "blur" }],
  ...
  mounted() {
    this.ActivityGroupId = this.$route.params.id;
  ...
  methods: {
    _sellerStoreList() {
      ...
      sellerStoreList(params,139).then(res => {
        this.options1 = res.List;
      });
    },
    selStore(e) {
      let temp = this.options1
        .filter(x => {
          return x.StoreId == e;
        })
        .map(x => {
          return x.ArrivalAmount;
        });
      if (temp.length <= 0) {
        this.addForm.ArrivalAmount = 0;
      } else {
        this.addForm.ArrivalAmount = temp[0];
      }
      //项目列表
      const params = {
        SellerId: 0,
      ...
      sellerItemList(params,139).then(res => {
        this.options2 = res.List;
      });
    },
    //购买须知
    _activitygroupBuynotesList() {
      activitygroupBuynotesList(null,139).then(res => {
        this.buynotes = res;
        res.forEach(item => {
    ...
    _activitygroupStoreAdd() {
      ...
      activitygroupStoreAdd(params,139).then(res => {
        this.$message({
          message: "添加成功",
          type: "success",
          showClose: true
