  ...
  <div>
    <div>
      ...
      >
        ...
        <div style="text-align:center;">
          <i
            ...
            v-if="common.haveRight('DeleteSheetImg')"
          ></i>
          <img :src="item.ImgAccessUrl" class="avatar" />
          <div v-if="common.haveRight('ModifySheetImgExamineStatus')">
            <el-button
        ...
        <div style="margin:0 50px;" v-if="common.haveRight('ModifySheetImgStatus')">
          <label>是否有效：</label>
          <el-switch
            v-model="item.Status"
            active-text="是"
      ...
      <el-upload
        ...
        v-if="common.haveRight('AddSheetImg')"
      >
...
import {
  ...
  AddSheetImg,
  DeleteSheetImg,
  ModifySheetImgStatus,
  ModifySheetImgExamineStatus
} from "api/sheet.js";
export default {
  name: "TeacherImgs",
  data() {
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      this._addSheetImg(res.Data.ImgPath).then(val => {
        this._getSheetImgList();
      });
    },
    //项目主图上传前
    ...
    deleteImg(item) {
      this.$confirm("此操作将永久删除该项, 是否继续?", "提示", {
        ...
        .then(() => {
          this._deleteSheetImg(item.ImgId).then(val => {
            this._getSheetImgList();
          });
        })
        .catch(() => {});
    ...
    statusChange(statue, item) {
      this._modifySheetImgStatus(item.ImgId, statue);
    },
    //显示审核弹窗
    ...
    //确认审核
    imgExamineOk() {
      this._modifySheetImgExamineStatus().then(val => {
    ...
    _getSheetImgList() {
      ...
      return GetSheetImgList(par, 68).then(res => {
        this.imgList = res;
      });
    },
    //添加师资图片
    ...
    _deleteSheetImg(id) {
      ...
      return DeleteSheetImg(par,72);
    },
    //修改师资图片状态
    _modifySheetImgStatus(id, status) {
      var par = {
        Id: id,
        Status: status
      };
      return ModifySheetImgStatus(par, 70);
    },
    //修改师资图片审核状态
    _modifySheetImgExamineStatus() {
      var par = {
        StatusDes: this.imgExamineForm.examineStatusDes,
        Status: this.imgExamineForm.examineStatus,
        Id: this.clickItem.ImgId
      };
      return ModifySheetImgExamineStatus(par, 71);
    }
  }
};
</script>
