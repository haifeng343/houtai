<template>
  <div>
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-button v-if="common.haveRight('AddSheetExtra')" type="primary" class="add" @click="add">添加团单</el-button>
    </el-form>
    <el-table :data="tableData" center style="width: 100%">
      ...
      <el-table-column label="是否有效" align="ceter">
        <template slot-scope="scope">
          <el-switch
            ...
            active-text="有效"
            inactive-text="无效"
            :disabled="!common.haveRight('ModifySheetExtraStatus')"
          ></el-switch>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button v-if="common.haveRight('ModifySheetExtra')" type="text" @click="edit(scope.row)">编辑</el-button>
          <el-button v-if="common.haveRight('DeleteSheetExtra')" type="text" class="red" @click="deleteData(scope.row.Id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    _extraAdd(F) {
      ...
      let Url = F == 1 ? extraModify : extraAdd;
      if (F == 1) {
        params.Id = this.addForm.Id;
      } else {
        params.SheetId = this.Id;
      }
      Url(params,F == 1?77:75).then(res => {
        this.$message({
    ...
    modifyStatus(item) {
      ...
      extraStatusModify(params,78).then(res => {
        this.$message({
          message: "修改成功",
          showClose: true,
          type: "success"
