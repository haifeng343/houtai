    ...
    <div>
      ...
      >
        ...
        <div style="text-align:center;">
          <i class="el-icon-close" @click="deleteImg(item)" v-if="common.haveRight('DeleteSellerStoreImg')"></i>
          <img :src="item.ImgAccessUrl" class="avatar"/>
          <div v-if="common.haveRight('ModifySellerStoreImgExamineStatus')">
            <el-button
        ...
        <div style="margin:0 50px;">
          ...
          <el-switch
            ...
            :disabled="!common.haveRight('ModifySellerStoreImgStatus')"
          ></el-switch>
        </div>
        <div>
          <label>上传时间：</label>
      ...
      >
        <i class="el-icon-plus avatar-uploader-icon" v-if="common.haveRight('AddSellerStoreImg')"></i>
      </el-upload>
    </div>
    <!-- 审核弹窗 -->
...
<script>
import {
  GetSellerStoreImgList,
  AddSellerStoreImg,
  DeleteSellerStoreImg,
  ModifySellerStoreImgStatus,
  ModifySellerStoreImgExamineStatus
} from "api/seller.js";
export default {
  name: "StoreImgs",
  data() {
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      this._addSellerStoreImg(res.Data.ImgPath).then(val => {
        this._getSellerStoreImgList();
      });
    },
    //项目主图上传前
    ...
    deleteImg(item) {
      this.$confirm("此操作将永久删除该项, 是否继续?", "提示", {
        ...
        .then(() => {
          this._deleteSellerStoreImg(item.ImgId).then(val => {
            this._getSellerStoreImgList();
          });
        })
        .catch(() => {});
    ...
    statusChange(statue, item) {
      this._modifySellerStoreImgStatus(item.ImgId, statue);
    },
    //显示审核弹窗
    ...
    //确认审核
    imgExamineOk() {
      this._modifySellerStoreImgExamineStatus().then(val => {
        this.imgExamineVisible = false;
        this._getSellerStoreImgList();
      });
    },
    //获取门店图片列表信息
    _getSellerStoreImgList() {
      var par = {
        Id: this.storeId
      };
      return GetSellerStoreImgList(par, 163).then(res => {
        this.imgList = res;
      });
    },
    //添加门店图片
    _addSellerStoreImg(imgurl) {
      var par = {
        StoreId: this.storeId,
        ImgUrl: imgurl
      };
      return AddSellerStoreImg(par, 164);
    },
    //删除门店图片
    _deleteSellerStoreImg(id) {
      var par = {
        Id: id
      };
      return DeleteSellerStoreImg(par,167);
    },
    //修改门店图片状态
    _modifySellerStoreImgStatus(id, status) {
      var par = {
        Id: id,
        Status: status
      };
      return ModifySellerStoreImgStatus(par, 165);
    },
    //修改门店图片审核状态
    _modifySellerStoreImgExamineStatus() {
      var par = {
        StatusDes: this.imgExamineForm.examineStatusDes,
        Status: this.imgExamineForm.examineStatus,
        Id: this.clickItem.ImgId
      };
      return ModifySellerStoreImgExamineStatus(par,166);
    }
  }
};
</script>
